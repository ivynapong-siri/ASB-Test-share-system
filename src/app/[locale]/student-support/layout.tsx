...
export default async function StudentSupportLayout({ children, params }: LayoutProps) {
  const { locale } = await params;

  const pageSetting = await fetchPageSettings({ locale });
  const studentSupportNavigation = pageSetting.pageSetting.menuGroups.studentSupport;

  const sharedSectionsData = await fetchSharedSections({ locale });
  const studentSupportSharedSections = (sharedSectionsData as { [key: string]: any })[
    "student-support"
  ] as SharedSectionJson[];

  return (
    <div className="relative">
      <SharedSectionsProvider sharedSection={studentSupportSharedSections}>
        {children}
        <DirectNavigationLinkButton options={studentSupportNavigation} />
