...
interface CommunityOutreachProgramSectionProps {
  data: SectionJson;
  breadcrumbData: BreadcrumbProps;
}

const CommunityOutreachProgramIntroSection = ({ data, breadcrumbData }: CommunityOutreachProgramSectionProps) => {
  return (
    <IntroSection
      title={data.title}
      ...
      vectorChildren={
        <>
          ...
          <PatternStroke1 className="absolute bottom-4 -left-18 z-10 h-19 w-42 rotate-35 max-md:translate-y-1/2 max-sm:top-auto lg:top-[700px] xl:top-[730px] xl:left-10 xl:h-38 xl:w-84 xl:-translate-x-1/3" />
        </>
      }
      containerClassName="sm:h-92 sm:w-88 xl:h-[596px] xl:w-[592px] transform scale-x-[-1]"
      topLeftIconClassName="-left-4 -top-4 sm:left-8 md:left-0 sm:top-3 lg:-left-3 xl:-left-1 transform scale-x-[-1]"
      topRightIconClassName="right-3 -top-2 sm:right-14 sm:top-7 md:top-2 md:right-8 lg:top-0 lg:right-8 xl:top-4"
      ...
      topLeftIcon={<CalendarIcon className="h-9 w-7 lg:h-[72px] lg:w-[51px]" />}
...
export default CommunityOutreachProgramIntroSection;
